...
class ProxylessFLOPsTable:
	...
	def build_lut(self, batch_size=1, resolutions=[112, 128, 144, 160, 176, 192, 208, 224]):
	...
	def build_single_lut(self, batch_size=1, base_resolution=224):
		...
		configurations = [
			...
			(MobileInvertedResidualBlock, base_resolution // 4, 24, 24, [3, 4, 6], [3, 5, 7], 1, 'relu', False),
			(MobileInvertedResidualBlock, base_resolution // 4, 24, 24, [3, 4, 6], [3, 5, 7], 1, 'relu', False),
			(MobileInvertedResidualBlock, base_resolution // 4, 24, 40, [3, 4, 6], [3, 5, 7], 2, 'relu', False),
			(MobileInvertedResidualBlock, base_resolution // 8, 40, 40, [3, 4, 6], [3, 5, 7], 1, 'relu', False),
			(MobileInvertedResidualBlock, base_resolution // 8, 40, 40, [3, 4, 6], [3, 5, 7], 1, 'relu', False),
			(MobileInvertedResidualBlock, base_resolution // 8, 40, 40, [3, 4, 6], [3, 5, 7], 1, 'relu', False),
			(MobileInvertedResidualBlock, base_resolution // 8, 40, 80, [3, 4, 6], [3, 5, 7], 2, 'relu', False),
			(MobileInvertedResidualBlock, base_resolution // 16, 80, 80, [3, 4, 6], [3, 5, 7], 1, 'relu', False),
			(MobileInvertedResidualBlock, base_resolution // 16, 80, 80, [3, 4, 6], [3, 5, 7], 1, 'relu', False),
			(MobileInvertedResidualBlock, base_resolution // 16, 80, 80, [3, 4, 6], [3, 5, 7], 1, 'relu', False),
			(MobileInvertedResidualBlock, base_resolution // 16, 80, 112, [3, 4, 6], [3, 5, 7], 1, 'relu', False),
			(MobileInvertedResidualBlock, base_resolution // 16, 112, 112, [3, 4, 6], [3, 5, 7], 1, 'relu', False),
			(MobileInvertedResidualBlock, base_resolution // 16, 112, 112, [3, 4, 6], [3, 5, 7], 1, 'relu', False),
			(MobileInvertedResidualBlock, base_resolution // 16, 112, 112, [3, 4, 6], [3, 5, 7], 1, 'relu', False),
			(MobileInvertedResidualBlock, base_resolution // 16, 112, 160, [3, 4, 6], [3, 5, 7], 2, 'relu', False),
			(MobileInvertedResidualBlock, base_resolution // 32, 160, 160, [3, 4, 6], [3, 5, 7], 1, 'relu', False),
			(MobileInvertedResidualBlock, base_resolution // 32, 160, 160, [3, 4, 6], [3, 5, 7], 1, 'relu', False),
			(MobileInvertedResidualBlock, base_resolution // 32, 160, 160, [3, 4, 6], [3, 5, 7], 1, 'relu', False),
			(ConvLayer, base_resolution // 32, 160, 960, 1, 1, 'relu'),
			(ConvLayer, 1, 960, 1280, 1, 1, 'relu'),
	...
	def predict_efficiency(self, sample):
		...
		unit_stats=[self.efficiency_dict[input_size]['mobile_inverted_blocks'][0][(3, 1)]  
					+ self.efficiency_dict[input_size]['other_blocks'][0]]  # first_conv + MBConv before first unit
		...
		unit_stats[5]=unit_stats[5]-unit_stats[4]
		unit_stats[4]=unit_stats[4]-unit_stats[3]
		unit_stats[3]=unit_stats[3]-unit_stats[2]
		unit_stats[2]=unit_stats[2]-unit_stats[1]
		# unit_stats =[ 1st conv, unit 0, unit 1, unit 2, unit 3, unit 4]

		return total_stats, unit_stats
